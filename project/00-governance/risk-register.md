...
| RISK-002 | Chromium/Firefox MAIN world 注入能力不一致 | High | Medium | High | Architect | 已采用 WXT `defineUnlistedScript` + `injectScript` + 最小 WAR；双浏览器构建/lint；后续补 Firefox 真浏览器 E2E | 任一浏览器无法安全运行 page-main | 降级为受支持能力集，推迟该浏览器 Stable | Mitigating |
