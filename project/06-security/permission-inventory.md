...
| `<all_urls>`                               | optional host permission  | Phase 2 仍只声明，尚未静默请求；实际内容脚本只匹配本地 fixture | 用户可选择按站点授权；不把全站访问写入 required host permissions | 用户明确授权后在选定网页控制媒体         | Chrome/Firefox 均由 permissions API 管理，授权 UX 在 Phase 3 适配 | manifest allowlist scan；权限 Port contract；双浏览器 core E2E 不依赖额外权限 | 若产品改为只支持 `activeTab` 单次启用或固定站点列表 |
| `http://localhost/*`、`http://127.0.0.1/*` | 静态 content-script match | 自动化 fixture 的 isolated + 声明式 MAIN-world runtime         | 仅用于可复现测试，不覆盖真实站点；page-main 不再使用 WAR         | 不面向商店用户                           | 两端一致                                                          | Chromium fixture E2E、Firefox 153 真扩展 core E2E、双浏览器 manifest 扫描     | 发布 profile 必须删除或明确标记 development-only    |
